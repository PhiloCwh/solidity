...
// File contracts/interfaces/ISwappiRouter01.sol
...
// File contracts/interfaces/ISwappiRouter02.sol

pragma solidity >=0.6.2;

interface ISwappiRouter02 is ISwappiRouter01 {
...
// File contracts/libraries/SwappiLibrary.sol
...
// File contracts/SwappiRouter.sol
...
contract SwappiRouter is ISwappiRouter02 {
    ...
    function _addLiquidity(
        ...
        if (ISwappiFactory(factory).getPair(tokenA, tokenB) == address(0)) {
            ISwappiFactory(factory).createPair(tokenA, tokenB);
        }
        (uint reserveA, uint reserveB) = SwappiLibrary.getReserves(factory, tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint amountBOptimal = SwappiLibrary.quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'SwappiRouter: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = SwappiLibrary.quote(amountBDesired, reserveB, reserveA);
                assert(amountAOptimal <= amountADesired);
                require(amountAOptimal >= amountAMin, 'SwappiRouter: INSUFFICIENT_A_AMOUNT');
    ...
    function addLiquidity(
        ...
        liquidity = ISwappiPair(pair).mint(to);
    }
    function addLiquidityETH(
        address token,
        uint amountTokenDesired,
    ...
    function removeLiquidity(
        ...
        address pair = SwappiLibrary.pairFor(factory, tokenA, tokenB);
        ISwappiPair(pair).transferFrom(msg.sender, pair, liquidity); // send liquidity to pair
        (uint amount0, uint amount1) = ISwappiPair(pair).burn(to);
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            ISwappiPair(SwappiLibrary.pairFor(factory, input, output)).swap(
